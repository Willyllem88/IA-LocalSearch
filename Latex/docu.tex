...
\begin{document}
	...
	Dos generados de solucions incials han estat pensats per resoldre aquest problema, cadascun maximitzant una aspecte difrent, un la felicitat del client i l'altre el preu del transport.
	
	\subsubsection{Maximitzar la felicitat}
	
	Per tal de maximitzar la felicitat haurem d'aconseguir assignar inicialment els paquets per tal de que s'entreguin tots el més aviat possible. Per això ordenarem la llista d'ofertes de menor dia d'entrega a major ($Ofertes$). Farem el mateix per les prioritats del paquet ($Paquets$). \\
	
	Un cop ambdues llistes estàn ordenades intentarem assignar els paquets de la següent manera. Vegeu a continuació el pseudocodi: \\
	
	\begin{verbatim}
	Per a cada paquet p_i que pertany a Paquets:
	    j = 0
	    Mentre j < nombre d'ofertes i p_i no estigui assignat:
	        Si p_i <= C_j:     # C_j és la capacitat de la oferta o_j
	            Assignar p_i a o_j
	        Altrament:
	            Incrementar j  # Provar amb la següent oferta o_{j+1}
	\end{verbatim}

	\subsubsection{Maximitzar el cost}
